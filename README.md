...
🚀 Key Features
Plug & Play Audit: Designed to run from a Raspberry Pi configured as a Mass Storage Gadget.
...
Configure Pi as Mass Storage: Set up your Raspberry Pi (Zero W or 4) to act as a USB Mass Storage Gadget using the g_mass_storage module.
Create Backing File: Create a container file (e.g., usb.bin) on the Pi and format it as NTFS or exFAT so Windows can read it.
Load Payload: Mount the container image locally on the Pi and copy all .ps1 files from this repository into the root of the image.
Deploy: Connect the Pi to the target PC via the USB data port. It will appear as a standard USB drive.
(Note: This suite also works perfectly on a standard USB flash drive.)
⚡ Usage
Insert the Raspberry Pi (or USB drive) into the target Windows machine.
Navigate to the drive in File Explorer.
Right-click run-audit.ps1 and select Run with PowerShell.
...
Wait for the message: "AUDIT COMPLETE. All reports saved.".
Open the PS_AUDITY_OUTPUT folder to view the HTML reports.
📊 Generated Reports
